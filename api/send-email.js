...
async function handleEmail({ type, to, toName, petName, link, message, ownerName, sitterName, tier, plan, report, note }) {
  ...
  switch (type) {
    case 'sitter_invite':
      subject = `${ownerName || 'Your friend'} invited you to care for ${petName || 'their pet'} on PupFile`;
      htmlContent = `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
          <h2 style="color:#ea580c">PupFile</h2>
          <p>Hi ${toName || 'there'},</p>
          <p><strong>${ownerName || 'Someone'}</strong> has invited you to help care for <strong>${petName || 'their pet'}</strong> on PupFile.</p>
    ...
    case 'support_reply':
      subject = `PupFile Support: ${escapeHtml(message) || 'We responded to your ticket'}`;
      htmlContent = `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
          <h2 style="color:#ea580c">PupFile Support</h2>
    ...
    case 'subscription':
      subject = `PupFile — you're now on the ${tier || 'paid'} plan!`;
      htmlContent = `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
          <h2 style="color:#ea580c">PupFile</h2>
          <p>Hi ${toName || 'there'},</p>
          <p>Your subscription to <strong>${tier || 'a paid'}</strong> plan is now active on PupFile.</p>
    ...
    case 'ai_report':
      subject = `AI Health Report — ${petName || 'Your Pet'} (PupFile)`;
      htmlContent = `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
          <h2 style="color:#ea580c">PupFile — AI Health Report</h2>
          ...
          <hr style="border:none;border-top:1px solid #eee;margin:16px 0">
          <p style="color:#999;font-size:0.75rem">Generated by PupFile — <a href="https://pupfile.com" style="color:#ea580c">pupfile.com</a></p>
        </div>`;
      break;
    ...
    case 'task_assignment':
      ...
      htmlContent = `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
          <h2 style="color:#ea580c">PupFile</h2>
          <p>Hi ${toName || 'there'},</p>
          <p><strong>${escapeHtml(ownerName || 'Someone')}</strong> assigned you a task for <strong>${escapeHtml(petName || 'your pet')}</strong>:</p>
          <div style="background:#f5f5f5;padding:16px;border-radius:8px;margin:12px 0;font-size:0.95rem">
            <strong>${escapeHtml(message || 'Unknown task')}</strong>
          </div>
          ${note ? `<p style="font-size:0.85rem;color:#666">${escapeHtml(note).replace(/\n/g, '<br>')}</p>` : ''}
          <a href="${link}" style="display:inline-block;padding:12px 24px;background:#ea580c;color:#fff;text-decoration:none;border-radius:8px;margin:16px 0">View in Dashboard</a>
          <p style="color:#666;font-size:0.85rem">Complete or manage this task from your PupFile dashboard.</p>
        </div>`;
      break;

    default:
      subject = body.subject || 'PupFile notification';
