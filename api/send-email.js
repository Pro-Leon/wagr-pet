...
async function handleEmail({ type, to, toName, petName, link, message, ownerName, sitterName, tier, plan, report, note }) {
  ...
  switch (type) {
    case 'sitter_invite':
      subject = `${ownerName || 'Your friend'} invited you to care for ${petName || 'their pet'} on Wagr`;
      htmlContent = `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
          <h2 style="color:#ea580c">Wagr</h2>
          <p>Hi ${toName || 'there'},</p>
          <p><strong>${ownerName || 'Someone'}</strong> has invited you to help care for <strong>${petName || 'their pet'}</strong> on Wagr.</p>
    ...
    case 'support_reply':
      subject = `Wagr Support: ${message || 'We responded to your ticket'}`;
      htmlContent = `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
          <h2 style="color:#ea580c">Wagr Support</h2>
          <p>Hi ${toName || 'there'},</p>
          <p>We've responded to your support ticket:</p>
          <div style="background:#f5f5f5;padding:16px;border-radius:8px;margin:12px 0;font-size:0.9rem">${(message || '').replace(/\n/g, '<br>')}</div>
          <p style="color:#666;font-size:0.85rem">You can view the full conversation in your dashboard.</p>
        </div>`;
      break;

    case 'welcome':
      subject = 'Welcome to Wagr — your pet care dashboard is ready';
      htmlContent = `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
          <h2 style="color:#ea580c">Wagr</h2>
          <p>Hi ${toName || 'there'},</p>
          <p>Welcome to Wagr! Your smart dog care dashboard is ready to go.</p>
          ...
          <a href="${link || 'https://wagr-ai.vercel.app/dashboard'}" style="display:inline-block;padding:12px 24px;background:#ea580c;color:#fff;text-decoration:none;border-radius:8px;margin:16px 0">Go to Dashboard</a>
          <p style="color:#666;font-size:0.85rem">Start with the Starter plan — free forever, no credit card needed.</p>
        </div>`;
      break;

    case 'subscription':
      subject = `Wagr — you're now on the ${tier || 'paid'} plan!`;
      htmlContent = `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
          <h2 style="color:#ea580c">Wagr</h2>
          <p>Hi ${toName || 'there'},</p>
          ...
          <p>You now have access to all features in your tier. <a href="https://wagr-ai.vercel.app/dashboard" style="color:#ea580c">Go to your dashboard</a> to start using them.</p>
          <p style="color:#666;font-size:0.85rem">Need help? Reply to this email or contact support from your dashboard.</p>
        </div>`;
      break;

    case 'ai_report':
      subject = `AI Health Report — ${petName || 'Your Pet'} (Wagr)`;
      htmlContent = `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
          <h2 style="color:#ea580c">Wagr — AI Health Report</h2>
          ...
          ${note ? `<hr style="border:none;border-top:1px solid #eee;margin:16px 0"><p style="font-size:0.88rem;color:#333"><strong>Note from owner:</strong><br>${note.replace(/\n/g, '<br>')}</p>` : ''}
          <hr style="border:none;border-top:1px solid #eee;margin:16px 0">
          <p style="color:#999;font-size:0.75rem">Generated by Wagr — <a href="https://wagr-ai.vercel.app" style="color:#ea580c">wagr-ai.vercel.app</a></p>
        </div>`;
      break;

    default:
      subject = body.subject || 'Wagr notification';
